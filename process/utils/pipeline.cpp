...
{
    m_client_ptr = std::make_unique<mongocxx::pool::entry>(MONGO.init_client());
    // 检查集合是否创建
    bool recv_is_exist = false;
    bool send_is_exist = false;
    for (const auto &coll : (*this->m_client_ptr)[m_db_name].list_collections())
    {
    ...
    // 获取集合
    this->m_send_coll_ptr = std::make_unique<mongocxx::collection>((*this->m_client_ptr)[m_db_name][m_send_name]);
    this->m_recv_coll_ptr = std::make_unique<mongocxx::collection>((*this->m_client_ptr)[m_db_name][m_recv_name]);
}
