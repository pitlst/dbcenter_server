...
def process(request_dataframe: pd.DataFrame, dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    for index, row in dataframe.iterrows():
        temp_qidian = str(row["点位1"]) if pd.notna(
            row["点位1"]) and pd.notnull(row["点位1"]) else ""
        temp_lianjiedian = str(row["连接点1"]) if str(row["连接点1"])[
            0] == "=" else "=" + str(row["连接点1"])
        ...
        temp_qidian = str(row["点位2"]) if pd.notna(
            row["点位2"]) and pd.notnull(row["点位2"]) else ""
        temp_lianjiedian = str(row["连接点2"]) if str(row["连接点2"])[
            0] == "=" else "=" + str(row["连接点2"])
        temp_str = temp_qidian + temp_lianjiedian
        if temp_str in no_have_list:
            len_inedx = len(one_img_info.index)
...
if st.button("开始检查"):
    with st.spinner('请稍等......正在处理......'):
        if not uploaded_file_request is None and not uploaded_file is None:
            df1 = pd.read_excel(uploaded_file_request,
                                sheet_name=0, skiprows=1, dtype=str)
