...
FROM 
    (SELECT mysql.udaProCode AS "项目号",
        ...
        mysql.fileCode AS "fileCode", 
        mysql.fileName AS "fileName", 
        ...
        END AS "prasePercent"
    FROM 
        (SELECT u.uda_pro_code AS udaProCode,
        ...
        GROUP BY  u.uda_pro_code, u.uda_car_code, u.code, vt.planned_start_time, vt.planned_finish_time, vt.gid,vt.dispatch_code, ml.code, ml.name, mo.op_code, mo.op_name, d.file_code, d.file_name, d.gid, d.car_type_code, dd.gid) mysql
        UNION ALL

        SELECT mysql.udaProCode AS "udaProCode",
         mysql.udaCarCode AS "udaCarCode",
         '校线' AS "type", mysql.orderCode AS "orderCode", mysql.dispatchCode AS "dispatchCode", mysql.id AS "id", mysql.plannedStartTime AS "plannedStartTime", mysql.plannedFinishTime AS "plannedFinishTime", mysql.lineCode AS "lineCode", mysql.lineName AS "lineName", mysql.opCode AS "opCode", mysql.opName AS "opName", '' AS "fileCode", '' AS "fileName", '' AS "carTypeCode", mysql.publishGid AS "publishGid",
            CASE
            WHEN mysql.completePercent = 0 THEN
            0
            WHEN mysql.completePercent = 100 THEN
        ...
        FROM 
         ...
         count(
            ...
            WHERE u.is_delete = 0 AND vt.planned_start_time >= SYSDATE - 180

                 
            GROUP BY  u.uda_pro_code, u.uda_car_code, u.code, vt.planned_start_time, vt.planned_finish_time, vt.gid,vt.dispatch_code, ml.code, ml.name, mo.op_code, mo.op_name, frs.gid) mysql
            ORDER BY  "opCode" ASC )