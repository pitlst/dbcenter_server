SELECT  bill.fid                                                                            AS "id"
       ...
       ,CASE bill.fk_crrc_isfirstcaryes WHEN 0 THEN '是' WHEN 1 THEN '否' END                 AS "是否完成首次确认"
       ...
       ,CASE bill.fk_crrc_jobstatus WHEN 'A' THEN '发送中' WHEN 'B' THEN '已下发' WHEN 'C' THEN '执行完工' WHEN 'D' THEN '质检合格' WHEN 'E' THEN '质检不合格' WHEN 'F' THEN '作废中' WHEN 'G' THEN '作废' WHEN 'H' THEN '转交中' END AS "任务状态"
       ,CASE bill.fk_crrc_jobstatus WHEN 'change' THEN '变更执行单' WHEN 'back' THEN '返工执行单' END AS "任务来源"
       ,bill.fk_crrc_reportbill                                                             AS "关联报告编号"
       ,CASE bill.fk_crrc_billstatus WHEN 15 THEN '评审完毕' WHEN 14 THEN '关闭' END              AS "状态"
       ,CASE bill.fk_crrc_transmit WHEN 'A' THEN '转交完成' END                                 AS "转交"
       ...
       ,CASE bill.fk_crrc_ischeck WHEN 0 THEN '是' WHEN 1 THEN '否' END                       AS "是否需要质检"
       ,CASE bill.fk_crrc_locksending WHEN 0 THEN '是' WHEN 1 THEN '否' END                   AS "锁定发送中"
       ,CASE bill.fk_crrc_issubmaterial WHEN 0 THEN '是' WHEN 1 THEN '否' END                 AS "是否退料"
       ,org2.FNAME                                                                          AS "所属组织"
...
FROM crrc_secd.tk_crrc_mesjob bill
LEFT JOIN crrc_sys.t_org_org org
ON bill.fk_crrc_deptid = org.FID
LEFT JOIN crrc_secd.tk_crrc_projectmanager project
ON bill.fk_crrc_projectid = project.fid
LEFT JOIN crrc_sys.t_org_org org1
ON bill.fk_crrc_departmentdpt = org1.FID
LEFT JOIN crrc_sys.t_sec_user create_user
ON bill.fk_crrc_createperson = create_user.FID
LEFT JOIN crrc_sys.t_org_org org2
ON bill.fk_crrc_orgfield = org2.FID
LEFT JOIN crrc_sys.t_sec_user execute_user
ON bill.fk_crrc_feedbackperson = execute_user.FID
LEFT JOIN crrc_sys.t_sec_user quality_user
ON bill.fk_crrc_closeuser = quality_user.FID